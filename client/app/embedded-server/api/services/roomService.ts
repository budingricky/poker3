...
class RoomService {
  ...
  joinRoom(roomId: string, playerName: string, playerId?: string): { player: Player; room: Room } {
    ...
    const offlineSameName = room.players.find(p => p.name === playerName && !p.isOnline)
    if (offlineSameName) {
      offlineSameName.isOnline = true
      offlineSameName.name = playerName
      return { player: offlineSameName, room }
    }

    const onlineCount = room.players.filter(p => p.isOnline).length
    if (onlineCount >= room.maxPlayers) throw new Error('房间已满');
    if (room.status === RoomStatus.PLAYING) throw new Error('游戏进行中，暂不可加入');
    ...
    const emptySeat = room.players.find(p => !p.isOnline)
    if (!emptySeat) throw new Error('房间已满')
    emptySeat.id = playerId || uuidv4()
    emptySeat.name = playerName
    emptySeat.roomId = roomId
  ...
  removePlayer(roomId: string, playerId: string): { roomDeleted: boolean, newHostId?: string } {
      ...
      if (room) {
          ...
          if (wasHost) {
            const nextHost = room.players.find(p => p.isOnline) || online[0]
            if (nextHost) {
